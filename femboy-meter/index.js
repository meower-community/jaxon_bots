...
const notes = 'ver0.2.1\nnotify command added + lucky number ooo'
    ...
    if (parsed) {
        if (parsed.command == 'meter') {
            if (db.data.hasOwnProperty(message.author)) {
                ...
                bot.post(`You are ${db.data[message.author].meter}% a femboy!`)
            } else {
            ...
            if (parsed.args[0]) {
                ...
                if (db.data.hasOwnProperty(user)) {
                    bot.post(`${user} is ${db.data[user].meter}% a femboy!`)
                } else {
                    bot.post(`${user} is not in the database!`)
                }
            }
        } else if (parsed.command == 'info') {
            bot.post(`Hosting ${Object.keys(db.data).length} users with a custom wrapper. // made by jaxonbaxon#7560`)
            ...
            if (db.data[message.author].lasttrade == 0 || (Math.floor(Date.now() / 1000) - db.data[message.author].lasttrade) >= 60) {
                if (db.data[message.author].meter >= 20) {
                    ...
                    bot.post(`Traded ${trade.sell} points off your femboy meter for ${trade.get} fembucks!`)
                } else {
                    bot.post('You don\'t have enough points on your femboy meter!')
                }
            } else {
                bot.post('Wait 1 minute until trading again!')
            }
            ...
            if (parsed.args.length > 0) {
                if (shop.hasOwnProperty(parsed.args[0])) {
                    if (db.data[message.author].fembucks >= shop[parsed.args[0]].price) {
                        ...
                        bot.post(`You bought ${parsed.args[0]}!`)
                    } else {
                        bot.post(`You don't have enough fembucks!`)
                    }
                } else {
                    bot.post(`That item doesn't exist!`)
                }
            } else {
                bot.post(`You didn't specify an item!`)
            }
        } else if (parsed.command == 'fembucks' && db.data.hasOwnProperty(message.author)) {
            bot.post(`You have ${db.data[message.author].fembucks} fembucks!`)
        } else if (parsed.command == 'items' && db.data.hasOwnProperty(message.author)) {
            if (db.data[message.author].items.length > 0) {
                bot.post(db.data[message.author].items.join(', '))
            } else {
                bot.post(`You don't have any items!`)
            }
            ...
            if (parsed.args[0]) {
                if (db.data[message.author].items.includes(parsed.args[0])) {
                    if (!db.data[message.author].equipped.includes(shop[parsed.args[0]])) {
                        ...
                        bot.post(use.replace(/#/g, ulist[Math.floor(Math.random() * ulist.length)]))
                    } else {
                        bot.post(`You can't equip that!`)
                    }
                } else {
                    bot.post(`You don't have that item!`)
                }
            }
            ...
            if (parsed.args[0]) {
                if (db.data[message.author].equipped.includes(parsed.args[0])) {
                    ...
                    if (shop[parsed.args[0]].hasOwnProperty('unequip-msg')) {
                        bot.post(shop[parsed.args[0]]['unequip-msg'][Math.floor(Math.random() * shop[parsed.args[0]]['unequip-msg'].length)])
                    } else {
                        bot.post(`You unequipped ${parsed.args[0]}.`)
                    }
                } else {
                    bot.post(`You don't have that item equipped!`)
                }
            }
        } else if (parsed.command == 'equipped' && db.data.hasOwnProperty(message.author)) {
            if (parsed.args[0]) {
                if (db.data[message.author].equipped.includes(parsed.args[0])) {
                    bot.post(`You have ${parsed.args[0]} equipped.`)
                } else {
                    bot.post(`You do not have ${parsed.args[0]} equipped.`)
                }
            } else {
                bot.post(db.data[message.author].equipped.join(', '))
            }
            ...
            bot.post('https://femboy-meter-docs.jaxonbaxon.repl.co')
            ...
            bot.post(`Set notifications to ${db.data[message.author].notify}`)
        }
        await db.write()
    }
})
